...
class YoutubeTabBaseInfoExtractor(YoutubeBaseInfoExtractor):
    ...
    def _extract_video(self, renderer):
        ...
        return {
            ...
            'timestamp': timestamp,
            'upload_date': strftime_or_none(timestamp),
    ...
    def _extract_lockup_view_model(self, view_model):
        ...
        metadata_rows = traverse_obj(view_model, (
            'metadata', 'lockupMetadataViewModel', 'metadata', 'contentMetadataViewModel', 'metadataRows'))
        metadata_parts = traverse_obj(metadata_rows, (..., 'metadataParts', ..., {dict}))
        metadata_part_texts = traverse_obj(metadata_parts, (..., 'text', 'content', {str}))

        timestamp = (traverse_obj(metadata_part_texts, (
            ..., {lambda t: self._parse_time_text(t, report_failure=False)}, any))
        ...
        if content_type == 'LOCKUP_CONTENT_TYPE_VIDEO':
            badges = self._extract_badges(traverse_obj(metadata_rows, (..., 'badges', ...)))
            live_status = None
            view_count = None
            concurrent_view_count = None

            for part in metadata_parts:
                text = traverse_obj(part, ('text', 'content', {str}), get_all=False)
                if not text:
                    continue
                text_lower = text.lower()
                count = 0 if 'no views' in text_lower else parse_count(text)
                if 'watching' in text_lower:
                    live_status = 'is_live'
                    concurrent_view_count = count
                elif 'waiting' in text_lower:
                    live_status = 'is_upcoming'
                    concurrent_view_count = count
                elif text_lower.startswith('scheduled for'):
                    live_status = live_status or 'is_upcoming'
                elif 'streamed' in text_lower:
                    live_status = live_status or 'was_live'
                elif (count is not None and ('view' in text_lower or traverse_obj(
                        part, ('leadingIcon', 'name', {str}), get_all=False) == 'PLAY_ARROW_OUTLINED')):
                    view_count = count
            ...
            info.update({
                'availability':
                    'public' if self._has_badge(badges, BadgeType.AVAILABILITY_PUBLIC)
                        ...
                        needs_subscription=self._has_badge(badges, BadgeType.AVAILABILITY_SUBSCRIPTION) or None,
                        is_unlisted=self._has_badge(badges, BadgeType.AVAILABILITY_UNLISTED) or None),
                view_count_field: concurrent_view_count if view_count_field == 'concurrent_view_count' else view_count,
                'live_status': live_status,
            })

        return self.url_result(
            url, ie, content_id, **info)
