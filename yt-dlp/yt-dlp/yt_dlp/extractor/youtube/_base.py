...
class YoutubeBaseInfoExtractor(InfoExtractor):
    ...
    def _extract_badges(self, badge_list: list):
        ...
        badges = []
        for badge in traverse_obj(badge_list, (
                ..., lambda key, _: re.search(r'(?:[bB]adgeRenderer|[bB]adgeViewModel)$', key))):
            badge_type = (
                icon_type_map.get(traverse_obj(badge, ('icon', 'iconType'), 'iconName', expected_type=str, get_all=False))
                or badge_style_map.get(traverse_obj(badge, 'style', 'badgeStyle', expected_type=str, get_all=False))
            )
            if badge_type:
            ...
            label = traverse_obj(
                badge, 'label', 'badgeText', 'accessibilityText', ('accessibilityData', 'label'),
                ('rendererContext', 'accessibilityContext', 'label'), 'tooltip', 'iconTooltip',
                get_all=False, expected_type=str, default='')
